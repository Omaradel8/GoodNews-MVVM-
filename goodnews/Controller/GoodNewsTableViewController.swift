...
class GoodNewsTableViewController: UITableViewController {
    ...
    func setup(){
        let url = URL(string: "https://newsapi.org/v2/top-headlines?country=us&apiKey=352ee9f1e1b54d0bb8273ee2159a916c")!
        service().getArticles(url: url){ articles in
            if let articles = articles{
                self.articleListVM = ArticleListViewModel(articles: articles.articles!)
    ...
    override func numberOfSections(in tableView: UITableView) -> Int {
        return articleListVM == nil ? 0 : articleListVM?.numberOfSections ?? 0
    }
